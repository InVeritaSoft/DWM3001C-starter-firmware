 ...
 * 
 * Supported Commands (orchestrator_v2):
 *   - PING (or PNG) → "OK"
 *   - NODE_TYPE → "OK NODE_TYPE=TX_V2" or "OK NODE_TYPE=RX_V2"
 *   - STRT (or START_TEST) → "OK START" (requires CONFIG first)
 *   - STOP (or STOP_TEST) → "OK STOP"
 *   - STAT (or GET_STATS) → "OK STATS sent=X attempted=Y errors=Z..."
 *   - CFG <params> (or SET_CONFIG <params>) → "OK CONFIG" (requires parameters, no defaults)
 *     Example: "CFG ch=5 rate=6m8 pl=128 len=64 pwr_ref=0x36363636 boost=30 rate_hz=100"
 *   - RST (or RESET_STATS) → "OK"
 * 
 * Communication Protocol:
 *   - Baud Rate: 115200 (Arduino SoftwareSerial)
 *   - DWM3001CDK firmware uses 30801920 internally but communicates at 115200
 *   - Commands sent with \r\n delimiter
 *   - Responses received with \r\n delimiter
 * 
 * Command Notes (orchestrator_v2):
 *   - CFG requires parameters (no default config like tcp_orchestrator)
 *   - START command is "STRT" or "START_TEST" (not "START")
 *   - STATS command is "STAT" or "GET_STATS" (not "STATS")
 *   - RESET_STATS command is "RST" or "RESET_STATS"
 * 
...
#define DWM_BAUD_RATE 115200  // orchestrator_v2 uses 30801920 internally, but Arduino SoftwareSerial works at 115200
...
void setup() {
    ...
    Serial.println("  - PING (or PNG)");
    Serial.println("  - NODE_TYPE");
    Serial.println("  - STRT (or START_TEST) - requires CONFIG first");
    Serial.println("  - STOP (or STOP_TEST)");
    Serial.println("  - STAT (or GET_STATS)");
    Serial.println("  - CFG <params> (or SET_CONFIG <params>)");
    Serial.println("    Example: CFG ch=5 rate=6m8 pl=128 len=64 pwr_ref=0x36363636 boost=30 rate_hz=100");
    Serial.println("  - RST (or RESET_STATS)");
    Serial.println("========================================");
    Serial.println();
}
