...
static void parse_command(char *cmd)
{
    ...
    else if (strcmp(cmd_upper, "RST") == 0 || strcmp(cmd_upper, "RESET_STATS") == 0)
    {
        ...
        g_seq_num = 0;
        g_consecutive_errors = 0; // Reset consecutive error counter
        memset(g_retransmit_queue, 0, sizeof(g_retransmit_queue));
        g_queue_head = 0;
        g_queue_tail = 0;
        g_queue_count = 0;
...
static void send_packet(void)
{
    ...
    g_tx_in_progress = 1;
    g_tx_in_progress_ticks = 0; // Reset watchdog counter
    ...
    // Check for TX success (matches working ex_22_orchestrator_v2 exactly)
    // Use status_reg from last loop iteration - don't re-read to avoid race conditions
    if (status_reg & DWT_INT_TXFRS_BIT_MASK)
    {
